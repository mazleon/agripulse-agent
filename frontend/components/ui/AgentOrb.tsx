...
export function AgentOrb({ status, onPressStart, onPressEnd }: AgentOrbProps) {
  const getOrbColor = () => {
    switch (status) {
      case "listening": return "rgba(57, 255, 20, 1)"; // Neon Green
      case "processing": return "rgba(0, 243, 255, 1)"; // Neon Blue
      case "alert": return "rgba(255, 0, 85, 1)"; // Neon Red
      default: return "rgba(57, 255, 20, 0.5)"; // Idle Green
    }
  };

  const getShadowColor = () => {
    switch (status) {
      case "listening": return "0 0 30px rgba(57, 255, 20, 0.8)";
      case "processing": return "0 0 30px rgba(0, 243, 255, 0.8)";
      case "alert": return "0 0 30px rgba(255, 0, 85, 0.8)";
      default: return "0 0 15px rgba(57, 255, 20, 0.3)";
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <motion.button
        ...
        animate={{
          scale: status === "listening" ? 1.1 : status === "processing" ? [1, 1.05, 1] : 1,
          boxShadow: getShadowColor(),
        }}
        transition={{
          scale: { duration: status === "processing" ? 1.5 : 0.2, repeat: status === "processing" ? Infinity : 0 },
        }}

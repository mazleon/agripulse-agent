...
export function VisionDiagnosis({ data }: { data: VisionData }) {
  ...
  const bgClass = isHighConfidence
    ...
    : isLowConfidence
      ? "bg-neon-red/20"
      : "bg-neon-yellow/20";
  const borderClass = isHighConfidence
    ? "border-neon-green"
    : isLowConfidence
      ? "border-neon-red"
      : "border-neon-yellow";
  ...
  return (
    ...
    >
      <div
        className={`px-4 py-2 flex items-center justify-between border-b ${borderClass} ${bgClass}`}
      >
        <div
          className={`flex items-center gap-2 text-sm font-bold ${colorClass}`}
        >
          {isHighConfidence ? (
            <CheckCircle size={16} />
          ) : (
        ...
        <TTSButton
          text={`${data.crop_type} গাছে ${data.disease_name} শনাক্ত হয়েছে। আত্মবিশ্বাস ${data.confidence} শতাংশ।`}
        />
      </div>
      ...
      <div className="p-4 flex gap-4">
        ...
        <div className="relative w-24 h-24 rounded-lg overflow-hidden shrink-0 border border-agri-600 bg-black">
          <img
            src={data.thumbnail_url || "/placeholder-leaf.jpg"}
            alt="Scanned crop"
            className="w-full h-full object-cover opacity-80"
          />
          <div
            className={`absolute top-2 left-2 right-2 bottom-2 border-2 ${borderClass} bg-transparent border-dashed animate-pulse`}
          />
        </div>
